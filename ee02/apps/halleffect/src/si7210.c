...
static char lora_payload[1];
    

static void alert1_irq(void *arg)
{
      lora_payload[0] = 1;
      lora_send(&lora_payload[0], sizeof(lora_payload));
      console_printf("Alert bottom.\n");
}

static void alert2_irq(void *arg)
{
      lora_payload[0] = 2;
      lora_send(&lora_payload[0], sizeof(lora_payload));
      console_printf("Alert middle.\n");
}

static void alert3_irq(void *arg)
{
      lora_payload[0] = 3;
      lora_send(&lora_payload[0], sizeof(lora_payload));
      console_printf("Alert top.\n");
}

void GPIOINT_Init()
{
    hal_gpio_init_in(7, HAL_GPIO_PULL_NONE);
    hal_gpio_init_in(8, HAL_GPIO_PULL_NONE);
    hal_gpio_init_in(11, HAL_GPIO_PULL_NONE);

    hal_gpio_irq_init(7, alert1_irq, NULL, HAL_GPIO_TRIG_FALLING, HAL_GPIO_PULL_NONE);
    hal_gpio_irq_init(8, alert2_irq, NULL, HAL_GPIO_TRIG_FALLING, HAL_GPIO_PULL_NONE);
    hal_gpio_irq_init(11, alert3_irq, NULL, HAL_GPIO_TRIG_FALLING, HAL_GPIO_PULL_NONE);

    hal_gpio_irq_enable(7);
    hal_gpio_irq_enable(8);
    hal_gpio_irq_enable(11);
}
...
/* HJG
   ...
   */
   ...
   // TODO: Parametriser buss-adresse
      int err = 0;
      uint8_t buffer[1];
      buffer[0] = addr;
      struct hal_i2c_master_data i2c_data_addr = {
            .address = SI7210_I2C_DEVICE_BUS_ADDRESS,
            .len = 1,
            .buffer = buffer
      };
      err = hal_i2c_master_write(I2CNUM, &i2c_data_addr, OS_TICKS_PER_SEC, 0);
      if (err != OS_OK) {
            console_printf("Chip: 0x%02x - Write register address (0x%02x) failed with error: '%d'\n", SI7210_I2C_DEVICE_BUS_ADDRESS, addr, err);
//            return err;
      }
      hal_i2c_master_write(I2CNUM, &i2c_data_addr, OS_TICKS_PER_SEC, 0);

      buffer[0] = 0;
      struct hal_i2c_master_data i2cData_data= {
            .address = SI7210_I2C_DEVICE_BUS_ADDRESS,
            .len = 1,
            .buffer = buffer,
      };

      err = hal_i2c_master_read(I2CNUM, &i2cData_data, OS_TICKS_PER_SEC, 1);
      if (err != OS_OK) {
            console_printf("Chip: 0x%02x - Read register 0x%02x failed with error: '%d'\n", SI7210_I2C_DEVICE_BUS_ADDRESS, addr, err);
//            return err;
      }
      *data = buffer[0];
...
*/
      ...
      buffer[0] = addr;
      buffer[1] = data;
      struct hal_i2c_master_data i2cData = {
            .address = SI7210_I2C_DEVICE_BUS_ADDRESS,
            .len = 2,
            .buffer = buffer
      };
      int err = hal_i2c_master_write(I2CNUM, &i2cData, OS_TICKS_PER_SEC, 1);

      os_time_delay(OS_TICKS_PER_SEC/5);

      uint8_t confirmedData;
      SI7210_regRead(addr, &confirmedData);

      if (data != confirmedData) {
            console_printf("Chip: 0x%02x - Write to register 0x%02x failed with error: '%d'\n", SI7210_I2C_DEVICE_BUS_ADDRESS, addr, err);
      }
      return SI7210_OK;
}
...
*/
      ...
      int err = hal_i2c_master_write(I2CNUM, &i2cData, OS_TICKS_PER_SEC, 1);
      if (err != OS_OK) {
            console_printf("Wakeup of 0x%02x failed with error: '%d'\n", SI7210_I2C_DEVICE_BUS_ADDRESS, err);
      }
...
static uint32_t loadCoeffsFromOtp( uint8_t otpAddr )
{
   ...
   int i;
   // HJG (retur av uinitialisert var...)
   uint32_t status = 0xFF; // because reasons...
   uint8_t value;
